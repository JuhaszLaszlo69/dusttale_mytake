...
@onready var animaciones: AnimatedSprite2D = $sprite
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("ui_up"): #----arriba
		...
		mira.set_target_position(Vector2(0,-50))
		animacion = "mov_arriba"
	elif mira.get_target_position() == Vector2(0,-50):
	...
	if Input.is_action_pressed("ui_left"):#-----izquierda
		...
		mira.set_target_position(Vector2(-50,0))
		animacion = "mov_izquierda"
	elif mira.get_target_position() == Vector2(-50,0):
