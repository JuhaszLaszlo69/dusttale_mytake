...
func _ready():
	...
	signalManager.connect("cell", Callable(self, "_on_cellphone_activated"))
	tie.connect("buff_end", Callable(self, "_on_buff_end"))
	tie.connect("enter_break", Callable(self, "_on_enter_break"))
	tie.connect("typesound", Callable(self, "_on_Typesound"))
...
func _on_buff_signal_activated(s):
	if s == "portrait hidden":
		tie.position = Vector2( 24, 22 )
		tie.size = Vector2( 560, 144 )
	if s == "portrait visible":
		tie.position = Vector2( 128, 22 )
		tie.size = Vector2( 432, 144 )
	if s == "comic sans":
		tie.set_font_bypath("res://addons/GodotTIE/Fonts/Comic-Sans-UT.tres")
	pass

var sans01_called = false
func sans01():
	if !dialogue.is_visible() && !sans01_called:
		...
		sans01_called = true
	elif !dialogue.is_visible() && sans01_called:
		$"/root/Root_Node/CanvasLayer/infoPanel/AudioStreamPlayer".stream = load("res://Assets/Sound Effects/PhoneRing.wav")
		$"/root/Root_Node/CanvasLayer/infoPanel/AudioStreamPlayer".play()
		tie.reset()
		tie.portrait_visible(false)
