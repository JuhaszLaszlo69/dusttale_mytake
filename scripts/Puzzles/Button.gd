...
func _ready():
	...
	if globalVariables.objectList.has(str(get_tree().current_scene.scene_file_path)): #continue if there are pressed buttons found in this scene
		aryDump = globalVariables.objectList[str(get_tree().current_scene.scene_file_path)].split(", ") #turn the array into a string
		if aryDump.find(self.name) != -1: #check if the array contains the current node's name
			$Sprite2D.set_frame(1) #button is visibly pressed
