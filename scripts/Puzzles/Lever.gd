...
@export var activationLocation: String = "" #Default_Value: "Current Scene" #A button can trigger something in a different scene with this setting # (String, FILE, "*.tscn")
var is_pulled := false
var aryDump : Array

func _ready():
	if !activationLocation: #if there's no alternate location set
		activationLocation = get_tree().current_scene.scene_file_path #set it to the current scene
	if globalVariables.objectList.has(str(get_tree().current_scene.scene_file_path)): #contiune if there are pulled levers found in this scene
		aryDump = globalVariables.objectList[str(get_tree().current_scene.scene_file_path)].split(", ")
		if aryDump.find(self.name) != -1: #check if self is one of the pulled levers
			$Sprite2D.set_frame(3) #lever is visibly pulled
...
func interaction_interact(interactionComponentParent : Node) -> void: #on lever pull attempt
	if !is_pulled: #if lever hasn't been pulled yet
		...
		$Sprite2D.set_frame(3) #lever is visibly pulled, frame 2 is the "unpulled" variant
		is_pulled = !is_pulled #is_pulled = true
		...
		if !globalVariables.objectList.has(str(get_tree().current_scene.scene_file_path)): #if there are no activated objects found in this scene
			globalVariables.objectList[str(get_tree().current_scene.scene_file_path)] = self.name #save new scene with new activated object name
		else: #if there are previously activated objects found in this scene
			globalVariables.objectList[str(get_tree().current_scene.scene_file_path)] += ", " + self.name #save this object along with all previous objects found in this scene
