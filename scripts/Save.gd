...
func _ready():
	signalManager.connect("buff_signal", Callable(self, "_on_buff_signal_activated"))
	signalManager.connect("save_update", Callable(self, "_on_save_update_activated"))
...
func ShowSavePanelWithSelector():
	...
	$HBoxContainer/Name.add_theme_color_override("font_color", Color(1,1,1))
	$HBoxContainer2/Level.add_theme_color_override("font_color", Color(1,1,1))
	$HBoxContainer3/Time.add_theme_color_override("font_color", Color(1,1,1))
	$Description.add_theme_color_override("font_color", Color(1,1,1))
	$"Option 1".add_theme_color_override("font_color", Color(1,1,1))
	$"Option 1".text = "Save"
...
func _input(ev):
	...
	if isActive && Input.is_action_just_pressed("move_left") || isActive && Input.is_action_just_pressed("move_right") || isActive && Input.is_action_just_pressed("interact"):
		...
		if ev.keycode == KEY_LEFT: # If left arrow is pressed
			if choice != 1:
				currentChoice.hide()
				choice -= 1
				currentChoice = get_node("Option " + str(choice) + "/Cursor")
				...
				currentChoice = get_node("Option " + str(choice) + "/Cursor")
				currentChoice.show()
		elif ev.keycode == KEY_RIGHT: # If right arrow is pressed
			if choice != 2:
				currentChoice.hide()
				choice += 1
				currentChoice = get_node("Option " + str(choice) + "/Cursor")
			...
			if choice == 1: #Save
				...
				$HBoxContainer/Name.add_theme_color_override("font_color", Color(1,1,0))
				$HBoxContainer2/Level.add_theme_color_override("font_color", Color(1,1,0))
				$HBoxContainer3/Time.add_theme_color_override("font_color", Color(1,1,0))
				$Description.add_theme_color_override("font_color", Color(1,1,0))
				$"Option 1".add_theme_color_override("font_color", Color(1,1,0))
