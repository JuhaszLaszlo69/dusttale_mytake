...
func _ready():
	...
	for item in globalVariables.talkList: #Initialize talk choices
		loopCount += 1
		var menuEntry = menuEntryScene.instantiate()
...
func updateSellMenu():
	if globalVariables.Items.size() > 0: # If you have more than 0 items
		...
		while loopCount != -1:
			if str(soldItems[loopCount]) == "SOLD_ITEM":
				globalVariables.Items.remove_at(loopCount) # remove sold items from inventory
			loopCount -= 1
	...
	for item in soldItems: #Initialize buy choices
		...
		if get_node_or_null("../InterPanel/sellGrid/sellOptions " + str(ceil(float(loopCount)/sellElementPerColumn))) == null:
			var sellOptionsEntry = sellOptionsScene.instantiate()
		...
		if str(item) != "SOLD_ITEM": #item isn't sold
			...
			tempNode.add_theme_color_override("font_color", Color(0.35,0.35,0.35))
...
func _input(ev):
	if !disableMenu:
		if Input.is_action_just_pressed("move_up") || Input.is_action_just_pressed("move_down") || Input.is_action_just_pressed("move_right") || Input.is_action_just_pressed("move_left") || Input.is_action_just_pressed("interact") || Input.is_action_just_pressed("open_menu"): # menu interactions
			if ev.keycode == KEY_UP && choiceExtendCopy != "../InterPanel/SellOptionHBox": # If up arrow is pressed
				updateChoice("up", choiceSize)
				if choiceExtend == "../InterPanel/buyOptions/":
					get_node("../../Root_Node").infoText(infoTextGen())
			elif ev.keycode == KEY_DOWN && choiceExtendCopy != "../InterPanel/SellOptionHBox": # If down arrow is pressed
				...
				pass
			elif ev.keycode == KEY_LEFT: # If left arrow is pressed
			...
			elif ev.keycode == KEY_C: # kind of like a back button
				if choiceExtend != "":
				...
				if choice == 1 && choiceExtend == "": # Buy Menu
					...
					SceneChanger.change_scene_to_file(globalVariables.exitShopScene, globalVariables.exitShopPosition, "fade")
					...
					if choice == 1: # sell confirmprompt Yes selected
						...
						get_node("../InterPanel/sellGrid/sellOptions " + str(sellColumnsChoice) + "/Option " + str(choiceRtn)).add_theme_color_override("font_color", Color(0.35,0.35,0.35))
